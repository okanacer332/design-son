...
import { useLanguage } from '@/src/lib/i18n/LanguageContext'; // Hook'u dahil ettik
...
export function Hero({ mode }: HeroProps) {
  const { t } = useLanguage(); // Çeviri nesnesini çekiyoruz
              ...
              <div className="space-y-6 sm:space-y-8">
                <div className="space-y-3 sm:space-y-4">
                  ...
                  <h1 className="text-5xl sm:text-6xl md:text-7xl lg:text-8xl xl:text-9xl bg-gradient-to-r from-purple-200 via-fuchsia-300 to-purple-400 bg-clip-text text-transparent leading-tight font-medium tracking-tight">
                    {t.hero.designTitle[0]} {/* DİNAMİK */}
                    <br />
                    {t.hero.designTitle[1]} {/* DİNAMİK */}
                    <br />
                    {t.hero.designTitle[2]} {/* DİNAMİK */}
                  </h1>
                </div>
                <p className="text-lg sm:text-xl md:text-2xl text-purple-200/80 max-w-2xl leading-relaxed">
                  {t.hero.designDesc} {/* DİNAMİK */}
                </p>
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-4">
                  <button className="px-8 py-4 bg-white text-purple-950 rounded-full hover:bg-purple-50 transition-all font-medium shadow-lg hover:shadow-purple-500/20 shadow-purple-500/10 w-full sm:w-auto">
                    {t.hero.designBtnPrimary} {/* DİNAMİK */}
                  </button>
                  <button className="px-8 py-4 bg-transparent text-white rounded-full hover:bg-white/5 transition-colors border border-white/10 w-full sm:w-auto">
                    {t.hero.designBtnSecondary} {/* DİNAMİK */}
                  </button>
                </div>
              </div>
            ) : (
              ...
              <div className="space-y-8">
                <div className="space-y-4">
                  ...
                  <h1 className="text-7xl md:text-8xl lg:text-9xl bg-gradient-to-r from-blue-200 via-cyan-300 to-blue-400 bg-clip-text text-transparent font-medium tracking-tight">
                    {t.hero.codeTitle[0]} {/* DİNAMİK */}
                    <br />
                    {t.hero.codeTitle[1]} {/* DİNAMİK */}
                    <br />
                    {t.hero.codeTitle[2]} {/* DİNAMİK */}
                  </h1>
                </div>
                <p className="text-xl md:text-2xl text-blue-200/80 max-w-2xl leading-relaxed">
                  {t.hero.codeDesc} {/* DİNAMİK */}
                </p>
                <div className="flex gap-4 flex-wrap pt-4">
                  <button className="px-8 py-4 bg-white text-blue-950 rounded-full hover:bg-blue-50 transition-all font-medium shadow-lg hover:shadow-blue-500/20 shadow-blue-500/10">
                    {t.hero.codeBtnPrimary} {/* DİNAMİK */}
                  </button>
                  <button className="px-8 py-4 bg-transparent text-white rounded-full hover:bg-white/5 transition-colors border border-white/10">
                    {t.hero.codeBtnSecondary} {/* DİNAMİK */}
                  </button>
                </div>
              </div>
            )}
